...
class Window : public Fl_Window { // az fltk -t fogjuk kibővíteni
public: 
	...
	void resize(int ww, int hh) { w=ww, h=hh; size(ww,hh); } // az ős osztály size fv je ez méretezi át

	void set_label(const string& s) { label(s.c_str()); } // Fl:Window tagja a label
...
private:
	  ...
	  void init(); // inicializáljuk az ablakot
}; 

int gui_main();	// invoke GUI library's main event loop |  a gui_main -nek a prototipusa
